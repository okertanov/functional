//
// Вчера пристально посмотрел на С++ Variadic Templates
// и неожиданно для себя изобрел RAII Scoped Resource Manager.
//
...
//    1. Инициализатор не вызывается перед конструктором, а в самом конструкторе.
//       Это, например, позволяет реализовать захват ресурса в lazy-стиле,
//       до первого вызова operator resource_t().
